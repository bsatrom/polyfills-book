...
Though it seems like a lot is going on here, a grunt file is pretty easy to parse once you get the hang of it. Basically, a gruntfile is made up of a series of named tasks, like +concat+, +uglify+ and the like. Each task tells grunt what actions to perform, and which files to perform those actions on when that task is executed. In this starter gruntfile, I've defined four tasks.
...
. +jshint+ - Performs static analysis on my JavaScript source files to make sure I'm following a consistent coding style.
