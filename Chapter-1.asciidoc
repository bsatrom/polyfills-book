...
Let's take a look at a simple, yet appropriate example: +String.trim()+. This useful function is included in all modern browsers, but it notably absent from Internet Explorer 6-8. If you're tasked with supporting these browsers, and you absolutely must trim your strings with a built-in function, you can add that needed functionality via a drop-in polyfill, as illustrated in <<EX4>>, courtesy of https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/Trim[Mozilla's excellent MDN documentation]
...
In this example, we're checking to see if the +trim+ function exists on the +String+ prototype and if not, it creates a new function that performs a simple RegEx replacement. To leverage this polyfill, a developer need to only include it somewhere in his app before the first call to +trim+. An important takeaway from this example is that first +if+ statement. If +trim+ does indeed exist, our polyfill isn't needed and thus, nothing else happens. It may seem like a minor point, but it's important to note that a good polyfill is aware of it's executing environment and only adds functionality when needed. We'll talk more about this in the next chapter.
...
Though CoffeeScript isn't a prollyfill per-se, it's a great example of allowing developer adoption to drive the standardization process, as evidenced by TC39's adoption of "fat arrow function syntax" for ECMAScript 6.
